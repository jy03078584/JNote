...
- 每个java对象中都持有Monitor锁对象，JVM使用monitorenter和monitorexit来实现同步处理。Java中使用Synchronized关键字标记方法/代码块需要同步。线程获取锁时对象的monitorenter+1，线程释放锁时monitorexit-1.
- Java中每个对象都能作为锁<br>
...
> 同步方法，锁是当前实例对象
  ...
  - 不会互斥其他线程对另一个对象的synchonized方法<br>

> 同步静态方法，锁是当前对象的Class对象<br>
  
  - 锁对象是Class
  - 不会互斥其他线程对该类的非static synchonized的访问<br>
...
- **ReentrantLock**
  1. ReentrantLock(可重入锁)，是唯一实现了Lock接口的类。
  2. Synchronized也是可重入锁，可重入锁简单理解：一个已经获取该对象的锁的线程在访问另一个加锁方法时不再需要重新申请锁
  ```java
  public synchronized void method1(){method2()}//访问method2时不需要再申请锁，否则永远等待
  public synchronized void method2(){...}
  ```
  3.原理
  ReentrantLock主要通过一个Sync(继承AbstractQueuedSynchronizer)
  的内部抽象类，以及它的2个实现类NoFairSync，FairSync。分别用于实现公平锁和非公平锁。
  ReentrantLock的核心是AQS实现：
  1. 主要提供exclusiveOwnerThread属性用于关联当前持有该锁的线程。
  2. 等待队列：对于等待获取锁的线程，AQS采用一个双向链表的队列存储。当多线程竞争锁时，采用CAS改变state的值，更改成功则成功获取锁。竞争失败的线程再次尝试获取锁仍然失败则进入等待队列。
  3. AQS的等待队列中尝试获取锁的机制很巧妙，因为是FIFO因此只需要NO1线程尝试获取锁，若失败则设置waitStatus为SIGNAL(-1)，当下次再发起尝试操作仍然失败后发现watiStatus已经为SINGAL则挂起线程。而队列后面的线程本身无需去尝试获取锁，只需要判断前面的线程waitStauts状态即可，若Prev线程为SINGAL则自己挂起。
  4. 释放锁/唤醒等待队列：当锁被释放后state重置为0,exclusiveOwnerThread=null。同时唤醒队列中首个被挂起的线程准备获取锁。
...
***
- **Lock与Synchronized比较**
　　1）Lock是接口(内部由CAS实现),而synchronized是Java中的关键字,synchronized是内置的语言实现,JVM对Synchronized有优化；
　　2）synchronized在发生异常时，会自动释放线程占有的锁，因此不会导致死锁现象发生；而Lock在发生异常时，如果没有主动通过unLock()去释放锁，则很可能造成死锁现象，因此使用Lock时需要在finally块中释放锁；
　　3）Lock可以让等待锁的线程响应中断，而synchronized却不行，使用synchronized时，等待的线程会一直等待下去，不能够响应中断；
　　4）通过Lock可以知道有没有成功获取锁，而synchronized却无法办到。
　　5）Lock可以提高多个线程进行读操作的效率。在性能上来说，如果竞争资源不激烈,Synchronized性能更好,而当竞争资源非常激烈时（即有大量线程同时竞争），此时Lock的性能要远远优于synchronized。
...
3. 调用wait(),notify(),notifyAll()必须在同步代码块中调用，因为调用者必须持有锁对象。虽不要求但wait()一般也都是在循环块中调用的，因为有时会出线伪唤醒的情况(一个wait线程被notify后还需要检查是否符合执行条件，不符合则需要再次被wait)
**调用对象的notify()后会唤醒等待线程，但仅仅是唤醒线程，并没有获得对象锁**
<br>**wait()会放弃对象锁；而Thread.sleep()只是暂停当前线程，让出CPU并不释放锁。**
<hr>
- Condition
Condition是JDK5后JUC包下提供用来替代wait()，在并发上更高效便捷。
...
### Java线程基础
  * 创建线程的方式 继承Thread，实现Runnable接口。相比继承Thread的方式，Runnable有特点:
    ...
    2)实现Runnable的方式能更方便的处理共享(继承Thread的方式中线程与任务是绑定在一起的，而线程只能启动一次，因此这种方式不同线程的资源也是不同的；而Runnable的方式是将任务交给线程处理，一个任务可交给多个任务处理)
...
####CountDownLatch
  ConntDownLatch位于JUC包下,利用计数的功能,实现一个线程等待其他线程完成操作后再执行的功能,不可重复使用
    ```java
...
####CyclicBarrier
  CyclicBarrier位于JUC包下,实现一组线程相互等待至某一状态后再统一放行。CyclicBarrier可以重复使用
    ```java
...
####Semaphore
  Semaphore位于JUC包下,能控制同一时刻并发的线程数。获取许可acquire后方能执行。
    ```java
